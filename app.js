...
(function(){
  ...
  const LS_KEY = "amm_enterprise_state_v4";
  const LS_SCEN = "amm_enterprise_scenarios_v4";
  ...
  function readStateFromUI(){
    ...
    return {P, APR, Y, m, START, MODE, EX, EXM, EXY, EXEND, extraCustom, INC_FIXED, INC_VAR, INC_MODE, INC_START, chartFlags};
  }
  ...
  function monthlyIncomeAt(state){
    // We treat each installment as "period" and income as monthly for simplicity when m=12.
    // When m != 12, we scale fixed/variable to the period length.
    const factor = 12 / state.m; // income per period = monthly * factor (e.g., m=6 => factor=2 months per period)
    ...
    return fixedPerPeriod + varPerPeriod;
  }
  ...
  function buildSchedule(state){
    ...
    const incomePerPeriod = monthlyIncomeAt(state);
  ...
  function wire(){
    ...
    const liveIds = ["P","APR","Y","m","START","MODE","EX","EXM","EXY","EXEND","INC_FIXED","INC_VAR","INC_MODE","INC_START"];
